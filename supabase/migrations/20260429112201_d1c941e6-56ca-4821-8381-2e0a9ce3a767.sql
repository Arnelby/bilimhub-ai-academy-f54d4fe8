...
CREATE VIEW public.research_user_metrics AS
WITH pre_tests AS (
  ...
  WHERE user_tests.test_id = '00000000-0000-0000-0000-000000000002'::uuid
    ...
    AND user_tests.total_questions > 0
    AND user_tests.score <= user_tests.total_questions
  ORDER BY user_tests.user_id,
    (user_tests.score::numeric / user_tests.total_questions::numeric) DESC,
    (COALESCE(user_tests.completed_at, user_tests.created_at)) DESC
), mid2_tests AS (
  SELECT DISTINCT ON (user_tests.user_id) user_tests.user_id,
    user_tests.score,
    user_tests.total_questions
  FROM user_tests
  WHERE user_tests.test_id = '00000000-0000-0000-0000-000000000003'::uuid
    AND user_tests.completed_at IS NOT NULL
    AND user_tests.score IS NOT NULL
    AND user_tests.total_questions IS NOT NULL
    AND user_tests.total_questions > 0
  ...
  SELECT practice_questions.user_id,
    count(*) AS practice_attempts
  FROM practice_questions
  GROUP BY practice_questions.user_id
), ai_stats AS (
  SELECT ai_chat_messages.user_id,
...
SELECT p.id AS user_id,
  ...
  CASE WHEN pre.total_questions > 0
    THEN round(pre.score::numeric / pre.total_questions::numeric * 100::numeric, 1)
    ELSE NULL::numeric END AS pre_score_pct,
  CASE WHEN mid1.total_questions > 0
    THEN round(mid1.score::numeric / mid1.total_questions::numeric * 100::numeric, 1)
    ELSE NULL::numeric END AS mid1_score_pct,
  CASE WHEN mid2.total_questions > 0
    THEN round(mid2.score::numeric / mid2.total_questions::numeric * 100::numeric, 1)
    ELSE NULL::numeric END AS mid2_score_pct,
  CASE WHEN post.total_questions > 0
    THEN round(post.score::numeric / post.total_questions::numeric * 100::numeric, 1)
    ELSE NULL::numeric END AS post_score_pct,
  CASE WHEN pre.total_questions > 0 AND post.total_questions > 0
    THEN round(post.score::numeric / post.total_questions::numeric * 100::numeric, 1)
       - round(pre.score::numeric / pre.total_questions::numeric * 100::numeric, 1)
...
FROM profiles p
  ...
  LEFT JOIN mid2_tests mid2 ON mid2.user_id = p.id
