...
CREATE TABLE IF NOT EXISTS public.practice_questions (
  ...
  topic text NOT NULL,
  question_type text NOT NULL DEFAULT 'comparison',
...
CREATE POLICY "Users can delete their own practice questions"
  ON public.practice_questions FOR DELETE
  USING (auth.uid() = user_id);

-- Recreate research views without group_type logic
...
WITH pre_tests AS (
  SELECT DISTINCT ON (user_id)
    user_id,
    CASE WHEN total_questions > 0 AND score <= total_questions
...
user_test_stats AS (
  SELECT
    ...
    sum(CASE WHEN score IS NOT NULL AND total_questions IS NOT NULL AND score <= total_questions THEN total_questions ELSE 0 END) AS total_questions_answered,
